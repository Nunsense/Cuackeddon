...
//			offset.z =i*myTransform.forward.z+Random.Range(-noise,noise)+myTransform.position.z;
			offset.x =target.position.x+Random.Range(-noise,noise);
			offset.z =target.position.z+Random.Range(-noise,noise);
			position[i] = offset;
...
//		Ray ray = new Ray(myTransform.position, target.position-myTransform.position);
		hit = Physics.RaycastAll(myTransform.position, target.position-myTransform.position, 350);
...
////				Move our End Effect particle system to the hit point and start playing it
				if(endEffect){
					endEffect.transform.position = hit [i].transform.position;
